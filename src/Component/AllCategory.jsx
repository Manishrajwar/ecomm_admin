...
function AllCategory({token , setSelectedItem ,setUpdateCategoryId ,updateSubCategoryId ,setUpdateSubCategoryId ,updateCategoryId ,updateProductId ,setUpdateProductId}){
    ...
    if(updateProductId !== null){
     ...
     const deleteSubCategory = async(subCatId) =>{
      ...
      try{
        ...
        const response = await fetch(`http://localhost:4000/api/v1//deleteSubCategory/${subCatId}` , {
          method:"DELETE",
          headers: {
            "content-type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        if(formattedResponse.success){
          toast.success("Successfuly delted the subCategory");
          fetchAllCategory();
        }
        else{
          toast.error(formattedResponse?.message)
...
<div class="relative overflow-x-auto">
  ...
  allCategory.map((category , index)=>(
    <tr key={index} onClick={()=>setSubCategory(category?.subCategory)} class=" cursor-pointer hover:bg-gray-900 transition-all duration-75 bg-white border-b dark:bg-gray-800 dark:border-gray-700">
...
<div class="relative overflow-x-auto">
  ...
  subCategory.map((subCat , index)=>(
    ...
    <td class="px-8 py-4">
       <img src={subCat?.images} className="max-w-[100px] max-h-[50px] mx-auto" alt="" />
    </td>

   
    <td class="px-8 py-4">
    <button type="button" onClick={()=>{
        sessionStorage.setItem("ecommAdmin_subCategoryId" ,subCat?._id );
    ...
    <td class="px-8 py-4 ">
    <button type="button" onClick={()=>deleteSubCategory(subCat._id)} className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete</button>
    </td>
